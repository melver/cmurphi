...
const
    ...
    MAX_D_U:             50;     -- max variation for user demand (kW) in sampling time;
...
type
    real_type:          real(10,99);  -- common real type
...
-- calculates the probability of the transition, depending on the distance from the middle value for the user demand
function user_distribution(usrdemand_d : Disturbance_usrdemand_type) : real_type;
var tmp : real_type;
begin
switch(usrdemand_d)
case -1 : 
	tmp := 0.4 + (((+ usrdemand - MAX_U/2)*fabs(usrdemand - MAX_U/2))/(MAX_U*MAX_U))*beta;
case 0  : 
	tmp := 0.2;
case 1  : 
	tmp := 0.4 + (((- usrdemand + MAX_U/2)*fabs(usrdemand - MAX_U/2))/(MAX_U*MAX_U))*beta;
endswitch;
return (tmp/((Modality_H - Modality_L + 1)*(Clamp_H - Clamp_L + 1)*(Disturbance_PI_type_H - Disturbance_PI_type_L + 1)*(Disturbance_type_H - Disturbance_type_L + 1)));
end;
...
pctl
[
	true UNTIL <= 900 ((v_rot < 40) | (v_rot > 120))
]<=0.001;
