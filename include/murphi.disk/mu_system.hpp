...
class StateManager {
  ...
  state_filtering_queue *queue;	// the queue for active states.
...
public:
  AlgorithmManager();
  ~AlgorithmManager();		//gdp: fixed the destructor chain
  void verify_bfs();
  void verify_dfs();
  void simulate();

  time_t start_time;		//gdp: less accurate time calculation... but includes disk times
};
