...
*/
  return (unsigned long) ((double) bytes /
			  ((state_filtering_queue::BytesForOneState_checked
			    () +
			    state_filtering_queue::BytesForOneState_unchecked
			    ()) * gPercentActiveStates +
			   state_filtering_queue::BytesForOneState_hashtable
			   ()));
}
...
/* AP: mu__real */
void mu__real::print_diff(state * prevstate)
{
  /* We assume that prevstate is not null. */
#ifndef ALIGN
