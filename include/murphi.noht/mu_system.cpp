...
:NumStates(NumStates),
statesCurrentLevel(0), statesNextLevel(0), currentLevel(0), pno(1.0) /* some vars might be cut, left because harmless */
{
#if __WORDSIZE == 64
...
{
  Error.Notrace("Internal Error, cannot print hashtable with -noht");
  the_states->print();
}

unsigned long StateManager::NumElts()
{
  return NumCurState + queue->NumElts();
}

unsigned long StateManager::NumEltsReduced()
{
  Error.Notrace("Internal Error, cannot print states reduced with -noht");
...
void StateManager::print_trace(StatePtr p)
{
  if (args->main_alg.mode == argmain_alg::Verify_bfs) {
    ...
    queue->print_trace_dfs(Rules->get_generator());
}
