...
debug_sym(FALSE, "debug symmetry")
{
  ...
  if (main_alg.mode == argmain_alg::Verify_bfs && print_trace.value)
    Error.Notrace("Cannot print counterexample in BFS without HT");
...
//         { /* if reenabled, take care that -noht is not given */
//        print_hash.set(TRUE);
//           continue;
//         }
 ...
 ReportManager::print_algorithm()
{
  ...
  cout << "Test driver: no visited states check\n\n";
}
